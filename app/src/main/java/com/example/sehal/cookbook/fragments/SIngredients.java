...
import com.example.sehal.cookbook.adapters.SIAdapter;
...
import static com.example.sehal.cookbook.misc.Keys.EndpointRecipe.KEY_INGREDIENTS;
import static com.example.sehal.cookbook.misc.Keys.EndpointRecipe.KEY_RECIPE;

/**
 * Created by sehal on 5/25/2015.
 */
public class SIngredients extends Fragment {
    ...
    private ArrayList<SIngredinetsInfo> recipelists = new ArrayList<>();
    SIAdapter siAdapter;
    ...
    public static SIngredients newInstance(String param1, String param2) {
        SIngredients fragment = new SIngredients();
    ...
    public void sendjsonrequest() {
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, getRequestUrl(10), new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                siAdapter.setIngredinetsInfo(parseJsonresponse(response));
            }
            ...
            public void onErrorResponse(VolleyError error) {
                if (error.getMessage() == null) {
                    ...
                    Toast.makeText(getActivity(), "RECIPE ING SEARCH ERROR " + error.getMessage(), Toast.LENGTH_SHORT).show();
                }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        siAdapter = new SIAdapter(getActivity());
        ringredients.setAdapter(siAdapter);

        return layout;
    }

    private ArrayList<SIngredinetsInfo> parseJsonresponse(JSONObject response) {
        ArrayList<SIngredinetsInfo> listrecipe = new ArrayList<>();
        if (response != null && response.length() > 0) {
            try {
                ...
                for (int i = 0; i < arrayrecipes.length(); i++) {
                    ...
                    SIngredinetsInfo ingredinetsInfo = new SIngredinetsInfo();
